...
class Project < ActiveRecord::Base
  ...
  has_many :project_members, -> { where(requested_at: nil) }, dependent: :delete_all, as: :source
  alias_method :members, :project_members
  has_many :users, through: :project_members

  has_many :requesters, -> { where.not(requested_at: nil) },
    dependent: :delete_all, as: :source, class_name: 'ProjectMember'
  ...
  has_many :lfs_objects, through: :lfs_objects_projects, dependent: :destroy
  ...
  has_many :commit_statuses, foreign_key: :project_id
  has_many :pipelines, class_name: 'Ci::Pipeline', foreign_key: :project_id
  has_many :builds, class_name: 'Ci::Build', foreign_key: :project_id # the builds are created from the commit_statuses
  has_many :runner_projects, class_name: 'Ci::RunnerProject', foreign_key: :project_id
  has_many :runners, through: :runner_projects, source: :runner, class_name: 'Ci::Runner'
  has_many :variables, class_name: 'Ci::Variable', foreign_key: :project_id
  has_many :triggers, class_name: 'Ci::Trigger', foreign_key: :project_id
  ...
  # Removes all associated artifacts and build traces.
  def remove_artifacts
    pattern = File.join(Gitlab.config.artifacts.path, '*', id.to_s)

    Dir.glob(pattern).each do |directory|
